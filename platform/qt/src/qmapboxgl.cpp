...
mbgl::ShapeAnnotationGeometry asMapboxGeometry(const QMapbox::ShapeAnnotationGeometry &geometry) {
    auto asLineString = [&](const QMapbox::ShapeAnnotationGeometry &lineString) {
        mbgl::LineString<double> mbglLineString;
        mbglLineString.reserve(lineString.geometry.first().first().size());
        for (const auto &coordinate : lineString.geometry.first().first()) {
            mbglLineString.emplace_back(mbgl::Point<double> { coordinate.second, coordinate.first });
        }
        return mbgl::ShapeAnnotationGeometry { mbglLineString };
    };

    auto asMultiLineString = [&](const QMapbox::ShapeAnnotationGeometry &multiLineString) {
        mbgl::MultiLineString<double> mbglMultiLineString;
        mbglMultiLineString.reserve(multiLineString.geometry.first().size());
        for (const auto &lineString : multiLineString.geometry.first()) {
            mbgl::LineString<double> mbglLineString;
            mbglLineString.reserve(lineString.size());
            for (const auto &coordinate : lineString) {
                mbglLineString.emplace_back(mbgl::Point<double> { coordinate.second, coordinate.first });
            }
            mbglMultiLineString.emplace_back(std::move(mbglLineString));
        }
        return mbgl::ShapeAnnotationGeometry { mbglMultiLineString };
    };

    auto asPolygon = [&](const QMapbox::ShapeAnnotationGeometry &polygon) {
        mbgl::Polygon<double> mbglPolygon;
        mbglPolygon.reserve(polygon.geometry.first().size());
        for (const auto &linearRing : polygon.geometry.first()) {
        ...
        return mbgl::ShapeAnnotationGeometry { mbglPolygon };
    };

    auto asMultiPolygon = [&](const QMapbox::ShapeAnnotationGeometry &multiPolygon) {
        mbgl::MultiPolygon<double> mbglMultiPolygon;
        mbglMultiPolygon.reserve(multiPolygon.geometry.size());
        for (const auto &polygon : multiPolygon.geometry) {
            mbgl::Polygon<double> mbglPolygon;
            ...
            mbglMultiPolygon.emplace_back(std::move(mbglPolygon));
        }
        return mbgl::ShapeAnnotationGeometry { mbglMultiPolygon };
    };

    switch (geometry.type) {
    case QMapbox::ShapeAnnotationGeometry::LineStringType:
        return asLineString(geometry);
    case QMapbox::ShapeAnnotationGeometry::PolygonType:
        return asPolygon(geometry);
    case QMapbox::ShapeAnnotationGeometry::MultiLineStringType:
        return asMultiLineString(geometry);
...
/*!
    Transforms a QMapbox::Annotation into mbgl::{Symbol,Line,Fill,StyleSourced}Annotation.
*/
mbgl::Annotation asMapboxGLAnnotation(const QMapbox::Annotation & annotation) {
    if (annotation.canConvert<QMapbox::SymbolAnnotation>()) {
        ...
        return mbgl::LineAnnotation { asMapboxGeometry(lineAnnotation.geometry), lineAnnotation.opacity, lineAnnotation.width, { *color } };
        ...
        if (fillAnnotation.outlineColor.canConvert<QColor>()) {
            ...
            return mbgl::FillAnnotation { asMapboxGeometry(fillAnnotation.geometry), fillAnnotation.opacity, { *color }, { *outlineColor } };
        } else {
            return mbgl::FillAnnotation { asMapboxGeometry(fillAnnotation.geometry), fillAnnotation.opacity, { *color }, {} };
        }
        ...
        return mbgl::StyleSourcedAnnotation { asMapboxGeometry(styleSourcedAnnotation.geometry), styleSourcedAnnotation.layerID.toStdString() };
    } else {
        return {};
    }
}
